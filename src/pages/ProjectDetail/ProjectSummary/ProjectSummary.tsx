...
import { type ProjectDetails, type ProjectDetailsById, type ProjectMember,type  ProjectRequirement } from '../../../services/projectApi';
  ...
  if (projectError || !project) {
    return (
      <div className="text-center py-10 text-red-600 bg-red-50 rounded-lg mx-auto max-w-md">
        ...
        <p className="font-semibold text-sm">{projectError ? `Error: ${(projectError as any)?.data?.message || 'Unknown error'}` : 'Project not found.'}</p>
        <button
  ...
  if (detailsError || !projectDetails) {
    return (
      <div className="text-center py-10 text-red-600 bg-red-50 rounded-lg mx-auto max-w-md">
        ...
        <p className="font-semibold text-sm">{detailsError ? `Error: ${(detailsError as any)?.data?.message || 'Unknown error'}` : 'Detailed project data not found.'}</p>
        <button
          onClick={() => window.location.reload()}
          className="mt-4 bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 transition text-xs"
        >
  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-7xl bg-gray-50 min-h-screen">
      ...
      <div className="bg-gradient-to-r from-white to-gray-100 shadow-xl rounded-2xl p-8 mb-8 border-l-4 border-blue-500 animate-fade-in">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-500 text-xs mt-1 font-medium">Project Key: {project.projectKey}</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
        ...
        <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-6 text-gray-700">
          <div className="space-y-2">
            <p className="flex items-center gap-2">
              ...
              <span className="font-semibold text-sm">Start Date:</span> <span className="text-xs">{formatDate(project.startDate)}</span>
            </p>
            <p className="flex items-center gap-2">
              <svg className="w-5 h-5 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
              </svg>
              <span className="font-semibold text-sm">End Date:</span> <span className="text-xs">{formatDate(project.endDate)}</span>
            </p>
          </div>
          <div className="space-y-2">
            <p className="flex items-center gap-2">
              ...
              <span className="font-semibold text-sm">Created:</span> <span className="text-xs">{formatDate(project.createdAt)}</span>
            </p>
            <p className="flex items-center gap-2">
              <svg className="w-5 h-5 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
              </svg>
              <span className="font-semibold text-sm">Last Updated:</span> <span className="text-xs">{formatDate(project.updatedAt)}</span>
            </p>
          </div>
        </div>
        <div className="mt-6 flex flex-col sm:flex-row gap-4 text-gray-700">
          <p className="flex items-center gap-2">
            ...
            <span className="font-semibold text-sm">Budget:</span> <span className="text-xs">{project.budget.toLocaleString()} VND</span>
          </p>
          <p className="flex items-center gap-2">
            <svg className="w-5 h-5 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"></path>
            </svg>
            <span className="font-semibold text-sm">Type:</span> <span className="text-xs">{project.projectType.replace('_', ' ')}</span>
          </p>
        </div>
      </div>
      ...
      <div className="bg-white shadow-xl rounded-2xl p-8 mb-8 border-l-4 border-indigo-500 animate-fade-in">
        <h2 className="text-xl font-semibold text-gray-900 mb-4 bg-gradient-to-r from-indigo-500 to-blue-500 text-transparent bg-clip-text">Description</h2>
      ...
      <div className="bg-white shadow-xl rounded-2xl p-8 mb-8 border-l-4 border-purple-500 animate-fade-in">
        <h2 className="text-xl font-semibold text-gray-900 mb-4 bg-gradient-to-r from-purple-500 to-pink-500 text-transparent bg-clip-text">Requirements</h2>
            ...
            <table className="w-full text-left border-separate border-spacing-y-2">
              ...
              <tbody>
                {projectDetails.requirements.map((req: ProjectRequirement, index) => (
                  <tr
                    key={req.id}
                    className={`border-b transition-colors ${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'} hover:bg-blue-50 hover:shadow-sm rounded-lg`}
                  >
      ...
      <div className="bg-white shadow-xl rounded-2xl p-8 border-l-4 border-green-500 animate-fade-in">
        <h2 className="text-xl font-semibold text-gray-900 mb-4 bg-gradient-to-r from-green-500 to-teal-500 text-transparent bg-clip-text">Team Members</h2>
        {projectDetails.projectMembers.length > 0 ? (
            ...
            {projectDetails.projectMembers.map((member: ProjectMember) => (
              ...
              >
                ...
                <div>
                  ...
                  <p className={getMemberStatusBadge(member.status)}>
                    Status: {member.status.replace('_', ' ')}
                  </p>
                  {member.joinedAt && (
                    <p className="text-gray-500 text-xs">Joined: {formatDate(member.joinedAt)}</p>
                  )}
                </div>
              </div>
            ))}
          ...
          <p className="text-gray-500 italic text-sm">No team members available{detailsError ? ': Error loading members' : ''}.</p>
        )}
      </div>
    </div>
  );
