...
import { useGetProjectDetailsByKeyQuery } from '../../../services/projectApi';
import { useGetByConfigKeyQuery } from '../../../services/systemConfigurationApi';
import { useGetTasksByProjectIdQuery } from '../../../services/taskApi'; // New import for tasks
import './ManualRiskModal.css';
...
const ManualRiskModal: React.FC<ManualRiskModalProps> = ({ onClose, onSave }) => {
  ...
  const [dueDate, setDueDate] = useState('');
  const [riskScope, setRiskScope] = useState('');
  const [taskId, setTaskId] = useState<string>(''); // New state for taskId
  const [errors, setErrors] = useState({
    title: '',
    riskScope: '',
    type: '',
    ...
    likelihood: '',
    dueDate: '',
    taskId: '', // New error field for taskId
  });
  ...
  const { data: tasksData, isLoading: isTasksLoading } = useGetTasksByProjectIdQuery(projectId!, { skip: skipMembers });
  ...
  const validateFields = () => {
    const newErrors = {
      ...
      dueDate: dueDate
        ? (new Date(dueDate) < new Date() ? 'Due date cannot be in the past' : projectEndDate && new Date(dueDate) > new Date(projectEndDate) ? 'Due date cannot exceed project end date' : '')
        : '',
      taskId: riskScope === 'TASK' && !taskId ? 'Task is required for task scope' : '',
    };
    setErrors(newErrors);
    return !Object.values(newErrors).some((error) => error);
  };
  ...
  if (isTypeLoading || isImpactLoading || isLikelihoodLoading || isScopeLoading || isProjectLoading || isTasksLoading) {
    return (
      <div className='manual-risk-modal-overlay fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50'>
        <div className='manual-risk-modal bg-white rounded-lg w-full max-w-md flex flex-col'>
          <div className='modal-header flex justify-between items-center p-4 border-b border-gray-200'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-4 items-center'>
              ...
              <div>
                <select
                  ...
                  onChange={(e) => {
                    setRiskScope(e.target.value);
                    setTaskId(''); // Reset taskId when scope changes
                    setErrors({
                      ...errors,
                      riskScope: !e.target.value
                        ? 'Risk scope is required'
                        ...
                        ? 'Invalid risk scope'
                        : '',
                      taskId: e.target.value === 'TASK' && !taskId ? 'Task is required for task scope' : '',
                    });
                  }}
                >
                  <option value=''>Select Risk Scope</option>
                  ...
                  <div>
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        setErrors({
                          ...errors,
                          taskId: !e.target.value ? 'Task is required for task scope' : '',
                        });
                      }}
                    >
