...
const MeetingRescheduleRequestSend = () => {
  ...
  const statusMap = useMemo(() => {
    const m = new Map<string, { label: string; color: string | null; orderIndex: number }>();
    statusResp?.data?.forEach((c) => {
      m.set(c.name, { label: c.label || c.name, color: c.color, orderIndex: c.orderIndex ?? 999 });
    });
    return m;
  ...
  // ==== system configs: pmNote max length ====
  ...
  const cfg = useMemo(() => {
    ...
    const pmNoteCfg = byKey.get('meetingReschedule_pmNote');
    const pmNoteMin = toInt(pmNoteCfg?.minValue ?? 0, 0);
    const pmNoteMax = toInt(pmNoteCfg?.maxValue ?? pmNoteCfg?.valueConfig ?? 1000, 1000);

    return {
      PMNOTE_MIN: pmNoteMin,
      PMNOTE_MAX: pmNoteMax,
    };
  }, [cfgResp]);
  ...
  const handleUpdateRequestStatus = async (req: any, statusName: string) => {
    const note = (pmNotes[req.id] || '').trim();

    // Validate note theo config
    if (note.length < (cfg.PMNOTE_MIN || 0)) {
      toast.error(`PM note must be ≥ ${cfg.PMNOTE_MIN} characters.`);
      return;
    }
    if (note.length > (cfg.PMNOTE_MAX || 1000)) {
      toast.error(`PM note must be ≤ ${cfg.PMNOTE_MAX} characters.`);
      return;
    }
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      ...
      {role !== 'CLIENT' && (
        <div className="mb-6">
          <RescheduleStatsChart requests={rawRequests || []} statusMap={statusMap} />
        </div>
      )}
                ...
                {(role === 'PROJECT_MANAGER' || role === 'TEAM_LEADER') && req.status === 'PENDING' && (
                  <>
                    <div className="flex items-center justify-between">
                      <label className="text-sm text-gray-700">PM note</label>
                      <span className="text-xs text-gray-500">
                        {noteVal.trim().length}/{cfg.PMNOTE_MAX}
                      </span>
                    </div>
                    <input
                      type="text"
                      placeholder={`Enter PM note (≤ ${cfg.PMNOTE_MAX} chars)`}
                      value={noteVal}
                      maxLength={cfg.PMNOTE_MAX}
                      onChange={(e) =>
                        setPmNotes((prev) => ({ ...prev, [req.id!]: e.target.value }))
                      }
                      className="w-full rounded border px-3 py-1 mb-2"
