...
interface NavItem {
  label: string;
  icon: React.ReactNode; // Changed from React.ReactNode to JSX.Element
  active?: boolean;
}

const ProjectDetailHeader: React.FC = () => {
  const [isPopupOpen, setIsPopupOpen] = useState(false);
  const [visibleTabs, setVisibleTabs] = useState<NavItem[]>(navItems); // Explicitly type the state
  ...
  useEffect(() => {
    const updateTabs = () => {
      if (containerRef.current && tabRef.current) {
        ...
        let totalWidth = 0;
        const newVisibleTabs: NavItem[] = [];
        const newHiddenTabs: NavItem[] = [];

        // Calculate the width of each tab dynamically
        navItems.forEach((item) => {
          const tab = document.createElement('li');
          tab.className = 'tab-item flex items-center';
          tab.innerHTML = `
          ...
          const tabWidth = tab.offsetWidth + 24; // Include gap (1rem = 16px + 8px padding)
          document.body.removeChild(tab);

          if (totalWidth + tabWidth <= containerWidth) {
            totalWidth += tabWidth;
            newVisibleTabs.push(item);
          } else {
  ...
  return (
    <div className="project-container">
      ...
      <nav aria-label="Project navigation" className="mt-4" ref={containerRef}>
        <ul ref={tabRef} className="nav-tabs">
