...
const MilestoneHeader: React.FC<MilestoneHeaderProps> = ({
  ...
  const { data: membersData, isLoading: membersLoading } = useGetProjectMembersWithPositionsQuery(
    projectId,
    {
  ...
  const handleSprintFilterChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const newSprintFilter =
      e.target.value === 'All' ? 'All' : e.target.value === '' ? null : e.target.value;
    setSprintFilter(newSprintFilter);
    onSortChange(dateFilter, newSprintFilter);
  };

  if (membersLoading) {
    return <div className='p-4 text-center text-gray-500 animate-pulse'>Loading...</div>;
  }
                ...
                <div className='flex space-x-2 overflow-x-auto max-w-xs py-2'>
                  {members.map((member) => (
                    <div key={member.id} className='relative group'>
                      ...
                      />
                      <span className='absolute top-full left-1/2 -translate-x-1/2 mt-2 px-2 py-1 text-xs bg-gray-800 text-white rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap'>
                        {member.name}
                      </span>
                    </div>
                  ))}
