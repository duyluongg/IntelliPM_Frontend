...
const ProjectTaskList: React.FC = () => {
  ...
  const tasks: TaskItem[] = isLoading || error || !workItemsData?.data ? [] : workItemsData.data.map((item) => {
    console.log('Item from API:', item); // Debug dữ liệu từ API
    return {
      id: item.key || '',
                  ...
                  {task.type === 'subtask' && task.taskId && task.taskId !== 'Unknown' ? (
                    <div className="subtask-key">
                      ...
                      <div className="key-bottom">
                        <svg className="subtask-link-icon" role="presentation" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path d="M4 6L8 10L12 6" stroke="#42526E" stroke-width="1.33333" fill="none"/>
                        </svg>
                        <span className="subtask-id" onClick={() => handleOpenWorkItem(task.key)}>{task.key}</span>
                      </div>
                    </div>
                  ) : (
                    <div className="task-key-wrapper">
                      <span className="task-id-small"></span>
                      <span className="task-key" onClick={() => handleOpenWorkItem(task.key)}>{task.key}</span>
                    </div>
                  )}
                </td>
                <td>{task.summary}</td>
                ...
                <td>
                  {task.comments > 0 ? (
                    <div className="comment-cell">
                      <svg role="presentation" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M2 2H14C14.5523 2 15 2.44772 15 3V11C15 11.5523 14.5523 12 14 12H6L2 15V3C2 2.44772 2.44772 2 3 2Z" stroke="#42526E" strokeWidth="1.33333" fill="none"/>
                        <circle cx="7" cy="6" r="1" stroke="#42526E" strokeWidth="1.33333" fill="none"/>
                        <circle cx="10" cy="6" r="1" stroke="#42526E" strokeWidth="1.33333" fill="none"/>
                      </svg>
