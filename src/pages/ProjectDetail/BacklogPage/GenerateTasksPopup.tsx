...
const GenerateTasksPopup: React.FC<GenerateTasksPopupProps> = ({
  ...
  const [generateTasks] = useGenerateTasksForSprintMutation();
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        tasks: tasks
          .filter((_, index) => selectedTasks.includes(`task-${index}`))
          .map((task) => ({
            projectId,
            title: task.title,
            type: task.type,
            ...
            dependencies: [], // Added to satisfy CreateTaskRequest interface
          })),
      };
            ...
            {tasks.length > 0 && (
              <div className="border border-gray-200 rounded-xl p-5 max-h-[50vh] overflow-y-auto bg-gray-50">
                ...
                {tasks.map((task, index) => (
                  <div key={`task-${index}`} className="flex items-center gap-3 mb-3">
                    <input
                      type="checkbox"
