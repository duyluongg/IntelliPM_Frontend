  ...
  useEffect(() => {
    const updateTabs = () => {
      if (containerRef.current) {
        ...
        const tabWidth = 100; // Approximate width per tab in pixels
  ...
  return (
    <div className="mx-16 pt-6 relative">
      ...
      <nav aria-label="Project navigation" className="mt-4 relative" ref={containerRef}>
        ...
        {isPopupOpen && hiddenTabs.length > 0 && moreButtonRef.current && (
          <div
            className="absolute z-10 w-48 bg-white border border-gray-200 rounded shadow-lg"
            style={{
              left: moreButtonRef.current.getBoundingClientRect().left + window.scrollX,
              top: moreButtonRef.current.getBoundingClientRect().bottom + window.scrollY,
            }}
          >
            <ul className="py-1">
