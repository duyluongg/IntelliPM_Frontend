...
useEffect(() => {
  ...
  const handleCreateMeeting = async () => {
    ...
    const meetingPayload = {
      ...
      attendees: finalParticipantIds.length,
      participantIds: finalParticipantIds,
      // status: cfg.STATUS_LIST?.[0] ?? 'ACTIVE',
    };
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {projectDetails.data.projectMembers
                  .filter((member: any) => member.accountId !== user?.id)
                          .filter((member: any) => {
            const role = user?.role as string;
            // Hide "client" account if the user is a Team Leader or Team Member
            if ((role === 'TEAM_LEADER' || role === 'TEAM_MEMBER') && member.username === 'client') {
                return false;
            }
            return true;
        })
