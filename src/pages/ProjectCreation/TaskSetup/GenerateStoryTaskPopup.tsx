...
import { useGenerateStoryTaskMutation, type StoryTaskResponse } from '../../../services/aiApi';
...
const GenerateStoryTaskPopup: React.FC<GenerateStoryTaskPopupProps> = ({
  ...
  const [storyTasks, setStoryTasks] = useState<StoryTaskResponse[]>([]);
  ...
  const handleGenerateStoryTasks = async () => {
    ...
    try {
      ...
      console.log('Generated story/task response:', response);

      if (!response.isSuccess) {
        throw new Error(response.message || 'Failed to generate stories/tasks');
      }

      setStoryTasks(response.data);
      setSelectedStoryTasks(response.data.map((_, index) => `item-${index}`));
    } catch (err: any) {
      console.error('Story/Task generation error:', err);
      setError(err?.data?.message || err.message || 'Failed to generate stories/tasks');
    } finally {
  ...
  const handleSaveItemEdit = (itemId: string) => {
    if (!itemTitle.trim()) {
      setError('Story/Task title is required.');
      return;
    }
  ...
  const handleSubmit = async () => {
    if (selectedStoryTasks.length === 0) {
      setError('Please select at least one story/task.');
      return;
    }
    ...
    try {
      ...
      // Validate all selected stories/tasks
      for (const item of selectedStoryTasksData) {
        if (!item.data.title.trim()) {
          throw new Error('All stories/tasks must have a title.');
        }
        if (item.data.startDate && item.data.endDate && new Date(item.data.endDate) < new Date(item.data.startDate)) {
          throw new Error(`Story/Task "${item.data.title}" has invalid dates: End date must be on or after start date.`);
        }
      }

      setEvaluationPayload(JSON.stringify({ storyTasks: selectedStoryTasksData }));
      setIsEvaluationPopupOpen(true);
      onStoryTasksGenerated(selectedStoryTasksData);
    } catch (err: any) {
      console.error('Submit stories/tasks error:', err);
      setError(err.message || 'Failed to process stories/tasks');
    } finally {
      ...
      <div className='fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60'>
        ...
        <div className='bg-white rounded-2xl shadow-2xl w-full max-w-4xl p-8 max-h-[85vh] overflow-y-auto'>
          <h2 className='text-2xl font-bold text-[#1c73fd] mb-6 flex items-center gap-3'>
            ...
            Generate Stories/Tasks for Epic
          </h2>

          <div className='mb-6 p-4 bg-blue-50 border border-blue-200 rounded-xl'>
            <p className='text-blue-800 text-sm'>
              <strong>Project:</strong> {projectKey} • <strong>Epic:</strong> {epicTitle} •{' '}
              <strong>Existing Titles:</strong> {existingTitles.length}
            </p>
            {existingTitles.length > 0 && (
              <div className='mt-2'>
                <p className='text-blue-700 text-xs mb-1'>Existing titles:</p>
                <div className='flex flex-wrap gap-2'>
                  ...
                  {existingTitles.length > 5 && (
                    <span className='text-xs text-blue-600'>+{existingTitles.length - 5} more</span>
                  )}
                </div>
              </div>
            )}
          ...
          {isGenerating || isGenerateStoryTaskLoading ? (
            <div className='flex justify-center items-center py-12 bg-white rounded-2xl shadow-md'>
              <div className='flex flex-col items-center gap-4'>
                ...
                >
                  Generating Stories/Tasks...
                </span>
                <p className='text-gray-600 text-sm text-center max-w-md'>
                  AI is analyzing your epic and creating meaningful stories/tasks for better project planning.
                </p>
              </div>
            </div>
          ) : (
              ...
              <div className='mb-6'>
                ...
                >
                  ...
                  <span className='text-lg'>
                    {isGenerating ? 'Generating Stories/Tasks...' : 'Generate Story/Task Suggestions'}
                  </span>
                </button>
              </div>
                ...
                <div className='border border-gray-200 rounded-xl p-6 max-h-[50vh] overflow-y-auto bg-gray-50'>
                  <div className='flex items-center justify-between mb-4'>
                    <h3 className='text-lg font-semibold text-gray-800'>
                      Generated Stories/Tasks ({storyTasks.length})
                    </h3>
                                ...
                                <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                                  ...
                                  <div className='md:col-span-2'>
                                    ...
                                    <textarea
                                      ...
                                      placeholder={`Enter ${item.type.toLowerCase()} description`}
                                    />
                                  </div>
                                  <div>
                                    <label className='block text-sm font-medium text-gray-700 mb-1'>
              ...
              <div className='flex justify-end gap-4 mt-8'>
                ...
                <button
                  ...
                  className={`px-6 py-3 text-sm font-semibold text-white rounded-xl transition-all duration-300 ${
                    isSubmitting || storyTasks.length === 0
                      ? 'bg-gray-500 opacity-70 cursor-not-allowed'
                      : 'bg-gradient-to-r from-[#1c73fd] to-[#4a90e2] hover:from-[#155ac7] hover:to-[#3e7ed1] hover:shadow-lg'
                  }`}
                  disabled={isSubmitting || storyTasks.length === 0}
                >
                  {isSubmitting ? 'Adding Stories/Tasks...' : `Add Selected Stories/Tasks (${selectedStoryTasks.length})`}
                </button>
              </div>
            </>
          )}
