...
const CompleteSprintPopup: React.FC<CompleteSprintPopupProps> = ({
  ...
  const handleConfirmMove = async () => {
    ...
    // Sử dụng JSX cho message
    const message = (
      <>
        Are you sure you want to <strong>complete sprint {sprintId}</strong> and{' '}
        <strong>move its {workItem} work item(s)</strong> to {targetName}? This action cannot be undone.
      </>
    );
  ...
  return (
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50'>
      <div className='bg-white rounded-lg shadow-lg w-[400px]'>
        <div className='bg-gradient-to-r from-blue-500 to-blue-700 text-white p-6 pb-10 rounded-t-lg relative'>
          ...
          {sprintError && (
            <p className='text-sm mt-2 text-red-200'>
              ...
              {(sprintError as any)?.data?.message || 'Unknown error'}
            </p>
          )}
        ...
        {/* Phần giữa: Chèn hình ảnh nhỏ, nằm giữa giao diện trắng và xanh */}
      ...
      <ConfirmationPopup
        ...
        message={
          <>
            Are you sure you want to <strong>complete sprint {sprintId}</strong> and{' '}
            <strong>move its {workItem} work item(s)</strong> to{' '} <strong>
            {selectedTarget === 'new_sprint'
              ? 'a new sprint'
              : selectedTarget === 'backlog'
              ? 'the backlog'
