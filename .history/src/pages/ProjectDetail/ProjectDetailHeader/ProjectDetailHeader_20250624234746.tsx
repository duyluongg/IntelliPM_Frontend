  ...
  useEffect(() => {
    const updateTabs = () => {
      if (containerRef.current && tabRef.current) {
        ...
        const tabWidth = tabRef.current.querySelector('li')?.offsetWidth || 100; // Dynamic tab width
        const maxVisible = Math.floor(containerWidth / tabWidth);

        if (maxVisible < navItems.length) {
          setVisibleTabs(navItems.slice(0, maxVisible - 1)); // -1 for "More" button
          setHiddenTabs(navItems.slice(maxVisible - 1));
        } else {
