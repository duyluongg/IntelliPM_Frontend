...
const ProjectDashboard: React.FC = () => {
    ...
    const doRefetches = async () => {
      try {
        await Promise.all([
          ...
          refetchRec(),
          refetchHistory(),
          showRecommendations ? triggerGetRecommendations(projectKey, true) : Promise.resolve(),
        ]);
      } catch (err) {
        console.error('❌ Error refetching dashboard data:', err);
      }
  ...
  useEffect(() => {
    setRecommendations(recData?.data ?? []);
    setApprovedIds([]); // Reset approvedIds when new recommendations are loaded
  }, [recData]);
  ...
  }> = ({ rec, index, projectId, approvedIds, setApprovedIds, setRecommendations }) => {
    const isApproved = approvedIds.includes(index);
    const [isEditing, setIsEditing] = useState(false);
    const [editedRec, setEditedRec] = useState<AIRecommendation>({ ...rec });
    const [errors, setErrors] = useState<{ recommendation?: string; type?: string; api?: string }>(
  ...
  const handleRefreshSuggestions = () => {
    triggerGetRecommendations(projectKey, true);
  };
