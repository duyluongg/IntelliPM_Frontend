...
type Props = {
  ...
  /** optional – nếu muốn control từ ngoài */
  open?: boolean;
...
const AddMembersModal: React.FC<Props> = ({
  ...
  const allChecked =
    filtered.length > 0 && filtered.every((m: any) => selected[m.accountId]);
  ...
  const handleAdd = async () => {
    ...
    const addedIds = Array.isArray((res as any)?.added) ? (res as any).added : [];
    onAdded?.(addedIds);

    closeModal();
  };

  // ---- Dialog wrapper (self-contained) ----
  const rootProps =
    typeof open === 'boolean'
      ? { open, onOpenChange }
      : { defaultOpen: true };
  ...
  return (
    <Dialog.Root {...rootProps}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 bg-black/50" />
        <Dialog.Content
          className="fixed left-1/2 top-1/2 z-[200] w-full max-w-xl -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white p-6 shadow-xl outline-none"
