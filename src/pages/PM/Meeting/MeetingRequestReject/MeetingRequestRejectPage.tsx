...
export default function MeetingRequestRejectPage() {
  const { user } = useAuth();
  ...
  const handleRequest = async (projectId: number) => {
    const { data, isSuccess } = await triggerGetProjectById(projectId, true);

    if (isSuccess && data?.data?.projectKey) {
      navigate(`/project/projects/form/PRIVATE/new?projectKey=${data.data.projectKey}`);
    } else {
      alert('Không lấy được projectKey từ projectId');
    }
  };
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
          {meetingsWithReject.map((meeting) => (
            <div key={meeting.id} className='bg-white p-4 rounded-lg shadow border border-gray-200'>
              ...
              <button
                onClick={() => handleRequest(meeting.projectId)}
